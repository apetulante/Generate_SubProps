...
int main(int argc, char ** argv)
{
   ...
   int len_main, num_subs;
   calc_subs(&len_main, &num_subs, a, ID, desc_scale, descID, num_prog, pid, Upid, desc_pid, phantom, SAM_mvir, mvir, rvir, r_s, 
   Vrms, mmp, last_MM_scale, Vmax, x_coord, y_coord, z_coord, x_vel, y_vel, z_vel, x_ang, y_ang, z_ang, spin, breadth_first_ID, 
   depth_first_ID, tree_root_ID, orig_halo_ID, snap_num, next_coprog_dep_ID, last_mainleaf_dep_ID, tidal_F, tidal_ID, r_s_klyp, 
   M200b_all, M200b_1, M200b_2, M200b_3, M200b_4, Xoff, Voff, spin_bull, b_to_a, c_to_a, A_x, A_y, A_z, T_U_rat, M_pe, halfmass_r, 
   misc_54, misc_55, misc_56, misc_57, misc_58, misc_59, misc_60, len_tree,
 ...
 //  printf("num_subs is: %d\n", num_subs);
   ...
   sub_merger_start = realloc(sub_merger_start, num_subs*sizeof(int));
   sub_merger_end = realloc(sub_merger_end, num_subs*sizeof(int));
   main_halo_start = realloc(main_halo_start, num_subs*sizeof(int));
   main_halo_end = realloc(main_halo_end, num_subs*sizeof(int));
   ...
   write_submerger(out_file, sub_merger_start, sub_merger_end, main_halo_start, main_halo_end, num_subs);
   fclose(out_file);
   free(filename);
